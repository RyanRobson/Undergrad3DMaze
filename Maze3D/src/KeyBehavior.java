...
public class KeyBehavior extends ViewPlatformBehavior
{
  ...
  */
  ...
  private int wkey = KeyEvent.VK_W;
  private int akey = KeyEvent.VK_A;
  private int skey = KeyEvent.VK_S;
  private int dkey = KeyEvent.VK_D;
  ...
  */
  {
    if((keycode == forwardKey) ||(keycode == wkey))
      moveBy(VFWD, FORWARD, VBACK);  // args: main camera, bird's eye, back camera
    else if((keycode == backKey) || (keycode == skey))
      moveBy(VBACK, BACK, VFWD);
    else if((keycode == leftKey) || (keycode == akey))
      doRotateY(ROT_AMT, LEFT);
    else if((keycode == rightKey) || (keycode == dkey))  
