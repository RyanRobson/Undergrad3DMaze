...
public class MazeManager 
{
  ...
  private final static String BLOCK_TEX = "images/wall.jpg";
  private final static String CYL_TEX = "images/cobbles.jpg";
  private final static String TRES_TEX = "images/treasure.jpg";
  private final static String EN_TEX  ="images/enemy.jpg";
  private final static String ST_TEX = "images/start.jpg";
  private final static String EX_TEX = "images/exit.jpg"; 


  private Appearance blockApp, cylApp, treasureApp,enemyApp,startApp, exitApp;   // obstacle appearances
  ...
  private void initialiseVars()
  {
    ...
    blockApp = makeApp(specular, BLOCK_TEX);    // blue texture for blocks
    cylApp = makeApp(medgreen, CYL_TEX);    // green texture for cylinders
    treasureApp = makeApp(specular, TRES_TEX);
    enemyApp = makeApp(specular, EN_TEX);
    startApp = makeApp(specular, ST_TEX);
    exitApp =  makeApp(specular, EX_TEX);
    ...
    for (int z=0; z<LEN; z++) { 
      for (int x=0; x<LEN; x++) {
        ...
        if (ch == 'S') {    // starting position
        	mazeBG.addChild(makeObs(ch,x,z,startApp));
        	//drawBlock(g,x,z);
          ...
          drawBlock(g, x, z);
            ...
            mazeBG.addChild( makeObs(ch, x, z, treasureApp) );
            //drawBlock(g, 0, 0);
        	
        }
        else if (ch == 'M'){
        	mazeBG.addChild(makeObs(ch,x,z,enemyApp));
        	//drawBlock(g, 0, 0);
        }
        else if (ch =='E'){
        	mazeBG.addChild(makeObs(ch,x,z,exitApp));
        	drawBlock(g,x,z);
  ...
  // place an obstacle (block/cylinder) at (x,z) with appearance app
  {
    ...
    if (ch == 'b')  // blue textured block
      obs = new Box(RADIUS, HEIGHT/2, RADIUS, 
    	   ...
    	   obs = new Box(RADIUS, HEIGHT, RADIUS, 
   				Primitive.GENERATE_TEXTURE_COORDS |
   				Primitive.GENERATE_NORMALS, app );
    else if (ch == 'T')
    obs = new Box(RADIUS/2, HEIGHT/2, RADIUS, 
			Primitive.GENERATE_TEXTURE_COORDS |
			Primitive.GENERATE_NORMALS, app );
    else if (ch == 'M')
        obs = new Box(RADIUS, HEIGHT/2, RADIUS, 
    	 ...
    	 obs = new Box(RADIUS, HEIGHT/6, RADIUS, 
     			Primitive.GENERATE_TEXTURE_COORDS |
     			Primitive.GENERATE_NORMALS, app );
    else if (ch =='E')
    	obs = new Box(RADIUS, HEIGHT/6, RADIUS, 
